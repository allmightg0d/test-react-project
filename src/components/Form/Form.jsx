...
export default function Register() {
    ...
    const checkout = {
      ...
      function generateTicket() {
        ...
        let customer = name;
        let mail = email;
        setPatata({ ...patata, titles: titl, prices: pric, client: customer, email: mail });
        setDisabled(false)
    ...
    //     e.preventDefault();
    //     console.log(`REGISTER RESPONDE ${data}`);
    // };
    ...
    return (
        ...
        <div className='formContainer'>
            <div className='formCard'>
              ...
              <div className="formItemsLeft">
                ...
                {cart.map(item => <li>{item.title} // <span> ${item.price * item.quantity} </span> </li>)}
                  </ul>
      ...
      {showTicket ?  <>  <h4>Hola, tu ticket es  {tick}</h4>
      <h3> <b> Nombre: {name} </b></h3>
      <h3>Email: {email} </h3>
